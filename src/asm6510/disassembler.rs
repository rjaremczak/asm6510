use super::{addrmode::AddrMode, memory::*, operation::Operation};
...
pub fn disassemble(memory: &Memory, pc: &mut u16) -> Columns {
    ...
    buf.2
        .push_str(&format!(" {} ", operation.instruction.to_string()));
    let opaddr = *pc + 1;
    buf.2.push_str(&match operation.addrmode {
        AddrMode::Implied => String::from(""),
        AddrMode::Relative => format!(
            "${:04X}",
            *pc as i32 + (memory.byte(opaddr) as i8) as i32 + 2
        ),
