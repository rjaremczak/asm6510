...
fn mode_asm(src: PathBuf, bin: Option<PathBuf>, dump_symbols: bool) -> Result<(), AppError> {
    ...
    if dump_symbols {
        ...
        symbols
            .iter()
            .for_each(|(k, v)| println!("\"{}\" = {:04X}", *k, *v as u16));
    }
...
fn mode_dis(start_addr: u16, end_addr: Option<u16>, bin: PathBuf) -> Result<(), AppError> {
    print!(
        "binary file {:?}, disassemble from address {:04X} ",
        bin, start_addr
    );
    match end_addr {
    ...
    asm6510::disassemble_file(start_addr, end_addr, bin)?
        .iter()
        .for_each(|cols| println!("{}{}{}", cols.0, cols.1, cols.2));
    Ok(())
}
...
fn main() {
    ...
    let res = match args.mode {
        Some(Mode::Asm {
            ...
            bin,
            dump_symbols,
        }) => mode_asm(src, bin, dump_symbols),
        Some(Mode::Dis {
            start_addr,
            end_addr,
            bin,
        }) => mode_dis(start_addr, end_addr, bin),
